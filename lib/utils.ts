...
export const truncateString = (
  ...
  const cutOffIndex = Math.min(text.length, maxLength);
  const subText = text.substring(0, cutOffIndex);

  for (let i = cutOffIndex - 5; i < cutOffIndex; i++) {
    if (text[i] === "." || text[i] === " ") {
...
export const truncateWord = (text: string, wordLimit: number): string => {
...
export const formatTimeandDate = (date: Date | undefined) => {
